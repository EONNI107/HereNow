...
export const GET = async (request: Request) => {
  ...
  try {
    const response = await tourApi.get(
      `locationBasedList1?numOfRows=10&MobileOS=ETC&MobileApp=HereNow&_type=json&arrange=Q&mapX=${latitude}&mapY=${longitude}&radius=4000&contentTypeId=${typeId}&serviceKey=${serviceKey}
    `,
    );
